...
fun ActionPanel(selection: MutableList<File>, sizeMap: MutableMap<String, Long>) {
    ...
    AnimatedVisibility(
        ...
        if (selection.isNotEmpty()) Surface(
            ...
            shape = RoundedCornerShape(30.dp),
            elevation = 5.dp,
            modifier = Modifier.padding(5.dp),/*.height(220.dp)*/
        ) {
            Column {
...
fun ActionPanelButton(icon: ImageVector, description: String, onClick: () -> Unit) {
    Surface(shape = RoundedCornerShape(15.dp), modifier = Modifier.padding(5.dp)) {
        IconButton({ onClick() }) { Icon(icon, description, Modifier.size(30.dp)) }
    }
}
